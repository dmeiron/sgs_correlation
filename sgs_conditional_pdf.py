...
# ── colour palette ─────────────────────────────────────────────────────────────
C = dict(exact="#2c7bb6", smag="#d7191c", dyn="#1a9641", fill="#abd9e9", grid="#cccccc")
...
def compute_conditional_pdfs(
    x, y, n_cond=5, n_tau_pts=200, x_quantiles=(0.1, 0.3, 0.5, 0.7, 0.9)
):
    """
...
def plot_conditional_mean_std(
    ax, stats, xlabel, model_x=None, model_y=None, label_model="Smagorinsky"
):
    c = stats["centres"]
    m = stats["mean"]
    s = stats["std"]
    ...
    ax.plot(
        c[ok], m[ok], color=C["exact"], lw=2, label=r"$\langle\tau_{12}|\bar{q}\rangle$"
    )
    ax.axhline(0, color="k", lw=0.7, ls="--")

    if model_x is not None and model_y is not None:
        ax.plot(model_x, model_y, color=C["smag"], lw=2, ls="--", label=label_model)
...
def plot_conditional_pdfs(ax, cond_pdfs, cmap_name="plasma"):
    ...
    for i, (xv, tau_grid, pdf) in enumerate(cond_pdfs):
        ...
        ax.plot(tau_grid, pdf, color=col, lw=1.8, label=f"$\\bar{{q}}={xv:.2f}$")
...
def plot_conditional_variance_ratio(ax, stats, xlabel):
    ...
    ax.set_xlabel(xlabel, fontsize=10)
    ax.set_ylabel(
        r"$\sigma(\tau|\bar{q})\,/\,|\langle\tau|\bar{q}\rangle|$", fontsize=10
    )
    ax.set_title(
        "Relative conditional std\n(< 1 → functional relation plausible)", fontsize=9
    )
...
def make_figure(data):
    ...
    hb0 = plot_joint_density(
        ax00,
        S12,
        tau,
        r"$\bar{S}_{12}$",
        r"$\tau_{12}$",
        r"Joint: $\tau_{12}$ vs $\bar{S}_{12}$",
    )
    hb1 = plot_joint_density(
        ax01,
        S_mag,
        tau,
        r"$|\bar{S}|$",
        r"$\tau_{12}$",
        r"Joint: $\tau_{12}$ vs $|\bar{S}|$",
    )
    hb2 = plot_joint_density(
        ax02,
        smag,
        tau,
        r"$\tau^{\rm Smag}_{12}$",
        r"$\tau_{12}$",
        r"Joint: exact vs Smagorinsky",
    )
    ...
    plot_conditional_mean_std(
        ax10, stats_S12, r"$\bar{S}_{12}$", model_x=s12_range, model_y=smag_curve
    )
    ax10.set_title(
        r"$\langle\tau_{12}|\bar{S}_{12}\rangle$", fontsize=10, fontweight="bold"
    )

    plot_conditional_mean_std(ax11, stats_Smag, r"$|\bar{S}|$")
    ax11.set_title(
        r"$\langle\tau_{12}||\bar{S}|\rangle$", fontsize=10, fontweight="bold"
    )

    plot_conditional_mean_std(ax12, stats_Omega, r"$|\bar{\Omega}|$")
    ax12.set_title(
        r"$\langle\tau_{12}||\bar{\Omega}|\rangle$", fontsize=10, fontweight="bold"
    )
    ...
    ax20.set_title(
        r"$p(\tau_{12}|\bar{S}_{12})$ for different $\bar{S}_{12}$ values",
        fontsize=10,
        fontweight="bold",
    )
    ...
    ax21.set_title(
        r"$p(\tau_{12}||\bar{S}|)$ for different $|\bar{S}|$ values",
        fontsize=10,
        fontweight="bold",
    )
    ...
    ax22.set_title(
        r"Normalised $p(\tau_{12}/\sigma||\bar{S}|)$"
    ...
    ax32.text(
        ...
        bbox=dict(
            boxstyle="round,pad=0.5", facecolor="#f7f7f7", edgecolor="#aaaaaa", lw=1.2
        ),
    )
...
def plot_delta_sensitivity(deltas=(0.5, 1.0, 2.0, 4.0), N=60_000, Cs=0.17):
    ...
    fig.suptitle(
        r"$p(\tau_{12}|\bar{S}_{12})$  for different filter widths $\Delta$",
        fontsize=12,
        fontweight="bold",
    )
    ...
    for ax, delta, col in zip(axes, deltas, colors_delta):
        ...
        tau = data["tau_12"]
        S12 = data["S_12"]
        cpdfs = compute_conditional_pdfs(
            S12, tau, n_cond=3, x_quantiles=(0.25, 0.5, 0.75)
        )
...
def main():
    ...
    parser.add_argument(
        "--out",
        default="sgs_conditional_pdf.png",
        help="Output filename for main figure",
    )
    parser.add_argument(
        "--out2",
        default="sgs_delta_sensitivity.png",
