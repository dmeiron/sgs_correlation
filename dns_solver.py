...
if not _USE_MPI:
    try:
        ...
        print("Warning: pyfftw not found. Install with  pip install pyfftw  "
...
def initial_condition(N, kx, ky, kz, k2, dealias, u_rms=1.0, k_peak=4, seed=42):
    ...
    def rand_h():
        r = (rng.standard_normal(sh) + 1j*rng.standard_normal(sh)).astype(np.complex64)
    ...
    ux_h *= scale; uy_h *= scale; uz_h *= scale
...
def nonlinear_and_force(ux_h, uy_h, uz_h, kx, ky, kz, k2, dealias,
                        N, eps_target, k_f):
    """
    ...
    fmask = (km <= k_f) & (km > 0)
    N6    = float(N**6)
    E_low = 0.5*float((np.abs(ux_h[fmask])**2 +
                        np.abs(uy_h[fmask])**2 +
                        np.abs(uz_h[fmask])**2).sum()) / N6
...
def rk3_step(ux_h, uy_h, uz_h, kx, ky, kz, k2, dealias,
             N, nu, dt, eps_target, k_f):
    ...
    for a, b, g in zip(_A, _B, _G):
        ...
        ux_h *= IF;  ux_h += b*dt*px
        uy_h *= IF;  uy_h += b*dt*py
        uz_h *= IF;  uz_h += b*dt*pz
...
def compute_stats(ux_h, uy_h, uz_h, k2, nu, N):
    ...
    wt  = np.ones_like(E2); wt[:,:,1:N//2] = 2.0
    ...
    k_mag  = np.sqrt(k2); Ek = E2*wt/(2*N6)
    km = k_mag.ravel(); Em = Ek.ravel(); pos = km>0
    idx = np.argsort(km[pos])
    L   = (np.pi/(2*urms**2+1e-30) *
           np.trapezoid((Em[pos]/(km[pos]+1e-30))[idx], km[pos][idx]))

    return dict(E=E, eps=eps, urms=urms, lambda_=lam, Re_lam=Re_lam,
                eta=eta, tau_k=tau_k, u_k=u_k, L=L,
                k_max_eta=(N//3)*eta)


def energy_spectrum(ux_h, uy_h, uz_h, k2, N):
    wt    = np.ones(k2.shape, np.float32); wt[:,:,1:N//2] = 2.0
...
def adaptive_dt(ux_h, uy_h, uz_h, k2, N, nu, CFL=0.4):
    S  = (N, N, N); dx = 2*np.pi/N
    ux = _irfftn(ux_h, s=S); uy = _irfftn(uy_h, s=S); uz = _irfftn(uz_h, s=S)
...
def compute_all_fields(ux_h, uy_h, uz_h, kx, ky, kz, k2, N, delta, ftype):
    ...
    def p(h): return _irfftn(h, s=S).astype(np.float32)
    def dp(fh, kd): return _irfftn(1j*kd*fh, s=S).real.astype(np.float32)
    def fp(a, b):
        return _irfftn(apply_filter(_rfftn(a*b).astype(np.complex64),
                                     k2, delta, ftype), s=S).astype(np.float32)

    ux=p(ux_h); uy=p(uy_h); uz=p(uz_h)
    fux=p(fux_h); fuy=p(fuy_h); fuz=p(fuz_h)
    ...
    # velocity gradients
    dudx=dp(fux_h,kx); dudy=dp(fux_h,ky); dudz=dp(fux_h,kz)
    dvdx=dp(fuy_h,kx); dvdy=dp(fuy_h,ky); dvdz=dp(fuy_h,kz)
    dwdx=dp(fuz_h,kx); dwdy=dp(fuz_h,ky); dwdz=dp(fuz_h,kz)

    # strain rate
    S_11=dudx; S_12=0.5*(dudy+dvdx); S_13=0.5*(dudz+dwdx)
    S_22=dvdy; S_23=0.5*(dvdz+dwdy); S_33=dwdz
    S_mag=np.sqrt(2*(S_11**2+S_22**2+S_33**2+2*(S_12**2+S_13**2+S_23**2)))

    # vorticity tensor
    O12=0.5*(dudy-dvdx); O13=0.5*(dudz-dwdx); O23=0.5*(dvdz-dwdy)
    ...
    r = lambda a: a.ravel()
    return dict(
        u=r(fux), v=r(fuy), w=r(fuz),
        tau_11=r(tau_11), tau_12=r(tau_12), tau_13=r(tau_13),
        tau_22=r(tau_22), tau_23=r(tau_23), tau_33=r(tau_33),
        tau_11d=r(tau_11d), tau_22d=r(tau_22d), tau_33d=r(tau_33d),
        tau_kk=r(tau_kk),
        S_11=r(S_11), S_12=r(S_12), S_13=r(S_13),
        S_22=r(S_22), S_23=r(S_23), S_33=r(S_33), S_mag=r(S_mag),
        Omega_12=r(O12), Omega_13=r(O13), Omega_23=r(O23), Omega_mag=r(Omag),
        omega_x=r(ox), omega_y=r(oy), omega_z=r(oz),
        k_res=r(k_res), Q=r(Q), tau_smag_12=r(tau_smag_12),
        delta=np.float32(delta), filter_type=ftype,
    )
...
def plot_energy_spectrum(k_sh, E_sh, stats, N, nu, out='dns_energy.png'):
    ...
    eta=stats['eta']; eps=stats['eps']
    kr = k_sh[(k_sh>5) & (k_sh*eta<0.5)]
    if len(kr)>2:
        ax.loglog(kr, 1.5*eps**(2/3)*kr**(-5/3), 'tomato', lw=1.8, ls='--',
                  label=r'$C_K\varepsilon^{2/3}k^{-5/3}$')

    ax.axvline(1/eta,  color='#777', ls=':',  lw=1.5,
               label=rf'$k\eta=1$  $(k\approx{1/eta:.0f})$')
    ax.axvline(N//3,   color='purple', ls='--', lw=1.2, alpha=0.6,
               label=f'Dealiasing cutoff $k={N//3}$')

    ax.set_xlabel('Wavenumber $k$', fontsize=12)
    ax.set_ylabel('$E(k)$', fontsize=12)
    ax.set_title(rf"DNS Energy Spectrum  —  $N={N}^3$,  $\nu={nu}$,  "
                 rf"$Re_{{\lambda}}={stats['Re_lam']:.0f}$,  "
                 rf"$k_{{max}}\eta={stats['k_max_eta']:.2f}$", fontsize=11)
...
def plot_time_history(history, out='dns_stats.png'):
    t=np.array(history['t']); E=np.array(history['E'])
    eps=np.array(history['eps']); Re=np.array(history['Re_lam'])
    keta=np.array(history['k_max_eta'])

    fig, axes = plt.subplots(4, 1, figsize=(9, 10), sharex=True)
    for ax,(d,lab,col) in zip(axes, [
        (E,   'TKE  $E$',                     'steelblue'),
        (eps, r'Dissipation  $\varepsilon$',   'tomato'),
        (Re,  r'$Re_\lambda$',                 'seagreen'),
        (keta,r'Resolution  $k_{max}\eta$',    'darkorange')]):
        ax.plot(t, d, color=col, lw=1.8); ax.set_ylabel(lab, fontsize=11)
        ax.grid(True, alpha=0.25)
    axes[3].axhline(1.0, color='k', ls='--', lw=1.2, label='$k_{max}\\eta=1$')
    axes[3].legend(fontsize=9); axes[3].set_xlabel('Time $t$', fontsize=11)
    fig.suptitle('DNS Time History', fontsize=13, fontweight='bold')
    fig.tight_layout(); fig.savefig(out, dpi=160); plt.close(fig)
    print(f"  Saved: {out}")


def plot_slice(ux_h, uy_h, uz_h, kx, ky, kz, k2, N, delta, ftype,
               out='dns_slice.png'):
    """Mid-plane (z=N//2) slices of |omega|, Q, |S|, |tau_12|."""
    S = (N, N, N); iz = N//2
    def dp(fh, kd): return _irfftn(1j*kd*fh, s=S).real
    def p(h):       return _irfftn(h, s=S).real
    def fp(a, b):
        return _irfftn(apply_filter(_rfftn(a*b).astype(np.complex64),
                                     k2, delta, ftype), s=S).real
    ...
    for ax, sl, ti, cm in zip(axes.ravel(), slices, titles, cmaps):
        ...
        ax.set_title(ti, fontsize=11); ax.set_xlabel('$x$'); ax.set_ylabel('$y$')
        plt.colorbar(im, ax=ax, shrink=0.85)
    fig.suptitle(f'Mid-plane slice  (z=N/2)  —  N={N}, Delta={delta:.3f}, {ftype} filter',
...
def run_dns(N=256, nu=8e-4, T=50.0, CFL=0.4,
            eps_target=0.1, k_f=2.5,
            delta_les=None, filter_type='sharp',
            checkpoint_in=None,
    ...
    print(f"  Backend  : {'pyfftw ('+str(_N_THREADS)+' threads)' if _USE_PYFFTW else 'numpy.fft'}")
    ...
    kz=kz.astype(np.float32); k2=k2.astype(np.float32)
    ...
    t=t0; t_end=t0+T; step=0
    history=dict(t=[], E=[], eps=[], Re_lam=[], k_max_eta=[])
    wall0=time.time(); next_diag=wall0

    st0=compute_stats(ux_h, uy_h, uz_h, k2, nu, N)
    print(f"  Initial:  E={st0['E']:.4f}  Re_lam~{st0['Re_lam']:.1f}  "
    ...
    while t < t_end:
        ...
        ux_h, uy_h, uz_h = rk3_step(
            ux_h, uy_h, uz_h, kx, ky, kz, k2, dealias,
            N, nu, dt, eps_target, k_f)
        t += dt; step += 1
        ...
        if wall >= next_diag or t >= t_end:
            ...
            history['t'].append(t); history['E'].append(st['E'])
            history['eps'].append(st['eps']); history['Re_lam'].append(st['Re_lam'])
            history['k_max_eta'].append(st['k_max_eta'])
            print(f"  {step:>7d}  {t:>8.3f}  {st['E']:>9.5f}  {st['eps']:>9.5f}  "
    ...
    np.savez_compressed(checkpoint_out, ux_h=ux_h, uy_h=uy_h, uz_h=uz_h, t=t, N=N, nu=nu)
    ...
    fields = compute_all_fields(ux_h, uy_h, uz_h, kx, ky, kz, k2, N,
                                 delta_les, filter_type)
    fields.update(dict(k_sh=k_sh, E_sh=E_sh,
                       nu=np.float32(nu), N=N,
...
if __name__ == '__main__':
    ...
    p.add_argument('--filter',  type=str,   default='sharp',
                   choices=['sharp','gaussian','tophat'], help='Filter type')
    ...
    p.add_argument('--ckpt',    type=str,   default='dns_checkpoint.npz',help='Checkpoint output')
    p.add_argument('--diag',    type=float, default=5.0, help='Diagnostic interval (wall-s)')
    a = p.parse_args()

    run_dns(N=a.N, nu=a.nu, T=a.T, CFL=a.CFL,
            eps_target=a.eps, k_f=a.kf,
            delta_les=a.delta, filter_type=a.filter,
            threads=a.threads, checkpoint_in=a.resume,
            checkpoint_out=a.ckpt, fields_out=a.out,
